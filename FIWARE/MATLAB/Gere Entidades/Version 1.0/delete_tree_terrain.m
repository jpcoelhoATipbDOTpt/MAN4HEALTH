function delete_tree_terrain(urn_terrain)
...
waitbar(0.2,h)
resposta=pesquisa_entidades_http_progress('refTerrain',urn_terrain,true);
urn_parcel=[];
if ~isempty(resposta)
...
for nprcl=1: n_parcel
    resposta=pesquisa_entidades_http_progress('refParcel',urn_parcel(nprcl,:),false);
    if ~isempty(resposta)
...
for ndvce=1: n_device
    resposta=pesquisa_entidades_http_progress('refDevice',urn_device(ndvce,:),false);
    if ~isempty(resposta)
