...
% Last Modified by GUIDE v2.5 05-Sep-2022 17:53:37
...
if ~isempty(x)
    urn=x(y,:);
    resposta=pesquisa_entidades_http('refTerrain',urn);
    if ~isempty(resposta)
...
if~isempty(x)
    urn=x(y,:);
    resposta=pesquisa_entidades_http('refParcel',urn);
    if ~isempty(resposta)
...
if ~isempty(x)
    urn=x(y,:);
    resposta=pesquisa_entidades_http('refDevice',urn);
...
if ~isempty(x)
    ...
    resposta=atributos_uma_entidade(urn);
...
% Começa por determinar quais os urn associados a esse terreno
...
for nter=1:n_terrain
    delete_tree_terrain(TextAsCells{1}{nter})
end
